<template>
	<view>
		...
		<view class="listView">
			...
			<view v-if="selectIndex==1&&collegeArr.length==0" class="none_showView">
			...
			<view v-if="selectIndex==2&&couserArr.length==0" class="none_showView">
				<image class="show_image" src="../../../static/image/myCertif/nodata@2x.png" mode=""></image>
			</view>
	...
	export default {
		...
		methods: {
			...
			getData(){
				...
				this.$api.post('myCourse!ajaxGetColectCourse.action',{
					loginKey:loginkey
				}).then(res=>{
					if(res.res.status ==0){
