...
<script>
	...
	export default {
		...
		onLoad(e) {
			...
			this.partExamRecordId = e.partExamRecordId

			if (this.partExamRecordId) {
				this.getRecordData()
			} else {
				this.getData()
			}
		...
		onUnload() {
			console.log("onUnload11")
			_this.subCommit()
		},

		onHide() {
			console.log("onhide11");
			_this.pushCommit()
		},
		onShow() {
		...
		methods: {
			getData() {
				...
				this.$api.post('college!ajaxGetExamPaper.action', {
					loginKey: loginKey,
					examId: _this.examId
				}).then(res => {
			...
			getRecordData() {
				let loginKey = uni.getStorageSync('loginKey')
				this.$api.post('college!ajaxGetPartExamRecord.action', {
					loginKey: loginKey,
					partExamRecordId: _this.partExamRecordId
				}).then(res => {
					...
					if (res.res.status == 0) {
						...
						for (var i = 0; i < _this.quesArr.length; i++) {

							let item = _this.quesArr[i]
							if (item.tag == true) {
								_this.tagNumber++
							}
			...
			pushCommit() { //保存进度
				...
				this.$api.post('college!ajaxAddPartExamRecord.action', {
					loginKey: loginKey,
					commitAnswers: data,
					examId: _this.examId
				}).then(res => {
			...
			subCommit() {
				...
				let data = JSON.stringify(obj)
				this.$api.post('college!ajaxCommitUserAnswers.action', {
					loginKey: loginKey,
					commitAnswers: data,
					examId: _this.examId
				}).then(res => {
